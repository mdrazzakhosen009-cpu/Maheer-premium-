...
- Store chatbot built in code with a real order-taking flow (name → phone → address → order number)
...
If Gemini is not configured, the coded store assistant still works; no chatbot API key is required for the local order-taking flow. For deployment, set ADMIN_PASSWORD to your own secure password.
