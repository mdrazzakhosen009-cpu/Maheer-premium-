...
app.post('/api/chat',async(req,res)=>{
 ...
 if(!msg)return res.json({reply:'Hi! I’m Maheer Assistant. Ask me about products, price, delivery, ordering or order tracking.'});
 if(process.env.GEMINI_API_KEY){try{const model=process.env.GEMINI_MODEL||'gemini-2.5-flash';const context='You are Maheer Store Assistant for a Bangladesh beauty and personal-care ecommerce store. Be concise, friendly and helpful. You can explain products, prices, categories, delivery/order steps. Never invent stock or policy details.';const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${process.env.GEMINI_API_KEY}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:context+'\nCustomer: '+msg}]}]})});const j=await r.json();const reply=j.candidates?.[0]?.content?.parts?.[0]?.text;if(reply)return res.json({reply,provider:'gemini'});}catch(e){}}
 const products=db?(await query('SELECT name,price,category FROM products ORDER BY featured DESC LIMIT 20')).rows:localProducts(); const low=msg.toLowerCase(); if(/hello|hi|assalam|হাই|হ্যালো/.test(low))return res.json({reply:'Welcome to MAHEER STORE ✨ Tell me what you’re looking for—skin care, hair care, makeup or gift sets.'}); if(/price|দাম|cost|কত/.test(low)){const p=products.find(x=>low.includes(String(x.name).toLowerCase().split(' ')[0])); if(p)return res.json({reply:`${p.name} এর দাম ৳${Number(p.price).toLocaleString('en-BD')}. চাইলে আমি order করার ধাপও বলে দিতে পারি।`}); return res.json({reply:'কোন product-এর দাম জানতে চান? Product name লিখুন।'});} if(/order|অর্ডার|কিনতে|buy/.test(low))return res.json({reply:'অর্ডার করতে Product → Buy Now/Add to Bag → Checkout এ গিয়ে নাম, ফোন ও ঠিকানা দিন। Cash on Delivery available in the demo flow.'}); if(/delivery|ডেলিভারি|shipping/.test(low))return res.json({reply:'Delivery charge/coverage Admin Panel থেকে পরিবর্তনযোগ্য। Checkout-এ আপনার ঠিকানা দিয়ে order confirm করুন।'}); const match=products.find(x=>low.includes(String(x.name).toLowerCase().split(' ')[0])); return res.json({reply:match?`${match.name} — ৳${Number(match.price).toLocaleString('en-BD')}. ${match.category} category. Product details খুলে rating, reviews ও order options দেখতে পারবেন।`:'আমি product, price, category, order এবং delivery বিষয়ে সাহায্য করতে পারি। আপনি কী খুঁজছেন?' });
});
